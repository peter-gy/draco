 ...
 */
function asp2vl(facts, url = DEFAULT_DATASET) {
    let mark = '';
    const encodings = {};
...
var util$4 = util$5.exports;

var TYPES = '__types__';

var PARSERS = {
  boolean: util$4.boolean,
  integer: util$4.number,
  number:  util$4.number,
  date:    util$4.date,
  string:  function(x) { return x == null || x === '' ? null : x + ''; }
};

var TESTS = {
  boolean: function(x) { return x==='true' || x==='false' || util$4.isBoolean(x); },
  integer: function(x) { return TESTS.number(x) && (x=+x) === ~~x; },
  number: function(x) { return !isNaN(+x) && !util$4.isDate(x); },
...
function type$1(values, f) {
  values = util$4.array(values);
  f = util$4.$(f);
  var v, i, n;
  ...
  if (values[TYPES]) {
    v = f(values[TYPES]);
    if (util$4.isString(v)) return v;
  }

  for (i=0, n=values.length; !util$4.isValid(v) && i<n; ++i) {
    v = f ? f(values[i]) : values[i];
  }

  return util$4.isDate(v) ? 'date' :
    util$4.isNumber(v)    ? 'number' :
    util$4.isBoolean(v)   ? 'boolean' :
    util$4.isString(v)    ? 'string' : null;
}

function typeAll(data, fields) {
  if (!data.length) return;
  var get = fields ? util$4.identity : (fields = fieldNames(data[0]), bracket);
  return fields.reduce(function(types, f) {
    return (types[f] = type$1(data, get(f)), types);
  }, {});
}
...
function infer(values, f, ignore) {
  ...
  for (i=0; i<values.length; ++i) {
    ...
    for (j=0; j<types.length; ++j) {
      if ((!ignore || !ignore.test(v)) && util$4.isValid(v) && !TESTS[types[j]](v)) {
        types.splice(j, 1);
        j -= 1;
      }
    }
...
type$1.annotation = annotation;
type$1.all = typeAll;
type$1.infer = infer;
type$1.inferAll = inferAll;
type$1.parsers = PARSERS;
var type_1 = type$1;
...
var util$3 = util$5.exports;
var d3_dsv = d3Dsv.exports;
...
dsv$1.delimiter = function(delim) {
  var fmt = {delimiter: delim};
  return function(data, format) {
    return dsv$1(data, format ? util$3.extend(format, fmt) : fmt);
  };
};

var dsv_1 = dsv$1;

var util$2 = util$5.exports;

var json$2 = function(data, format) {
...
var util$1 = util$5.exports,
    d3_time = d3Time.exports,
    d3_timeF = d3TimeFormat.exports,
    d3_numberF = d3Format.exports,
    numberF = d3_numberF, // defaults to EN-US
    timeF$1 = d3_timeF,     // defaults to EN-US
...
var format = {
  ...
  time:         function(f) { return timeF$1.format(f); },
  utc:          function(f) { return timeF$1.utcFormat(f); },
...
function numberLocale(l) {
  var f = util$1.isString(l) ? d3_numberF[localeRef(l)] : d3_numberF.locale(l);
  if (f == null) throw Error('Unrecognized locale: ' + l);
  numberF = f;
}

function timeLocale(l) {
  var f = util$1.isString(l) ? d3_timeF[localeRef(l)] : d3_timeF.locale(l);
  if (f == null) throw Error('Unrecognized locale: ' + l);
  timeF$1 = f;
...
function monthFormat(month, abbreviate) {
  var f = abbreviate ?
    (monthAbbr || (monthAbbr = timeF$1.format('%b'))) :
    (monthFull || (monthFull = timeF$1.format('%B')));
  return (tmpDate.setMonth(month), f(tmpDate));
}

function dayFormat(day, abbreviate) {
  var f = abbreviate ?
    (dayAbbr || (dayAbbr = timeF$1.format('%a'))) :
    (dayFull || (dayFull = timeF$1.format('%A')));
